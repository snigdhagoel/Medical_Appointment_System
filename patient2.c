...
int main(int argc, char *argv[])
{
    ...
    // the credentials for patient2 are read from patient2.txt

    credentials = fopen("patient2.txt","r");
    if(NULL == credentials)
    {
	fprintf(stderr, "patient2.txt file not found.\n");
	return 1;
    }
	...
	*/
    ...
    // read the patient's insurance from patient2insurance.txt
    FILE *insuranceFile;
    insuranceFile = fopen("patient2insurance.txt", "r");
...
(my_addr.sin_port), inet_ntoa(*addr_list[0]));
    ...
    printf("End of Phase 3 for Patient 2.\n");
    close(sockfd);
    return 0;
}
