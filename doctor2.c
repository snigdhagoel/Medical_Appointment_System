...
#define MYPORT "42515"   // the static UDP port the patients will be connecting to
...
int main(void)
{
    ...
    FILE *docFile;
    // insurances is used to hold the information read from doc2.txt
    char **insurances;
    int index = 0;
    docFile = fopen("doc2.txt", "r");
    insurances = (char **)malloc(3 * sizeof (char *));
    for(index = 0; index < 3; index++)
    {
	/* doc2.txt contains entries of the form 'insurance1 30'
	...
	*/
	...
	/* We compare the insurance received with the content read from
	   'doc2.txt' file. We send back the corresponding cost to the 
	   patient.
	*/
	for(index = 0; index < 3; index++)
	{
